    ...
    extends AbstractTableModel
{
     ...
     * 
     */
    private static final long serialVersionUID = -2200545768070025629L;
        ...
        public void run() {
            while (true) {
                ...
                synchronized (mLock) {
                    ...
                    final Iterator<EventDetails> it = mPendingEvents.iterator();
                    while (it.hasNext()) {
                        final EventDetails event = it.next();
    ...
    private final SortedSet<EventDetails> mAllEvents = new TreeSet<EventDetails>(MY_COMP);
    ...
    /** @see javax.swing.table.TableModel **/
    public Class<? extends Object> getColumnClass(int aCol) {
        // does not need to be synchronized
    ...
    private void updateFilteredEvents(boolean aInsertedToFront) {
        ...
        final List<EventDetails> filtered = new ArrayList<EventDetails>();
        final int size = mAllEvents.size();
        final Iterator<EventDetails> it = mAllEvents.iterator();
        ...
        final EventDetails lastFirst = (mFilteredEvents.length == 0)
            ? null
            : mFilteredEvents[0];
        mFilteredEvents = filtered.toArray(EMPTY_LIST);
